...
export function Contact() {
  ...
  function validate() {
    if (form.name.trim().length < 2) return t.contact.errName;
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(form.email)) return t.contact.errEmail;
    if (form.message.trim().length < 5) return t.contact.errMsg;
    return "";
  }
          ...
          <ul className="space-y-3">
            ...
            <li>
              ...
              >
                ...
                <span>
                  <span className="block text-xs text-muted">{t.contact.phoneLabel}</span>
            ...
            <li>
              ...
              >
                ...
                <span>
                  <span className="block text-xs text-muted">{t.contact.codeLabel}</span>
            ...
            <li>
              ...
              >
                ...
                <span>
                  <span className="block text-xs text-muted">{t.contact.networkLabel}</span>
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4 rounded-2xl border border-border glass p-6 shadow-xl shadow-primary/5">
          ...
          <div>
            ...
            <input id="c-name" type="text" className={inputClass} value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={t.contact.namePh} />
          </div>
          <div>
            ...
            <input id="c-email" type="email" className={inputClass} value={form.email}
              onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder={t.contact.emailPh} />
          </div>
          <div>
            <label htmlFor="c-message" className="mb-1.5 block text-sm font-medium">{t.contact.message}</label>
            <textarea id="c-message" rows={5} className={inputClass} value={form.message}
              onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder={t.contact.messagePh} />
          </div>
          ...
          {status === "ok" && (
            <p className="text-sm font-medium text-primary" role="status">{t.contact.ok}</p>
          )}
          {status === "error" && (
